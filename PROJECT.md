...
- `solver_tree_fh.py` — **1v1-солвер на полном дереве** с perfect-recall
  инфосетами (CFR+). Абстракция «удары до смерти» (`hits = ceil(hp / per-hit)`).
...
- `run_full_history.py` — запуск полного 1v1-солвера (`solver_tree_fh`).
- `run_table.py`, `export_1v1_table.py` — генерация/экспорт таблиц.
- `gen_value_data.py`, `train_value_net.py`, `value_leaf.py` — ReBeL:
  генерация данных, обучение value-net, листовое значение сети.
- `train_pool.py` — удалён (обучал старый солвер).
...
- `test_1v1_solver_vs_planner.py` — 1v1-солвер против Planner.
...
- `server_out_fh/` — чекпоинты полного 1v1-солвера (cap6).
...
rules.py ── эталон правил (игра, харнессы, вера)
   ...
   ├── solver_tree_fh.py ── точный 1v1 (эталон значений, лаборатория)
...
- **Модель урона Rust vs rules.py**: Rust-движок/солверы используют
  отсечение per-hit (`floor(atk·mult/10)` в единицах по 10 HP, абстракция
  «удары»), а `rules.py` — округление к 100 (`round(atk·mult·hits/100)·100`).
  Для `atk·mult`, кратных 100, они совпадают; иначе расходятся на десятки HP.
  Авторитетная по `RULES.md` — округление к 100. **Решение отложено**:
  либо оставить как аппроксимацию солвера, либо править Rust-движок и
  пересобрать таблицу.
