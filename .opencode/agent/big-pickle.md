---
description: Test subject (FREE - big-pickle) that plays COTE Megaverse against the bot and reports a win rate. Use when launching a subagent to play the game via play.py with --run big-pickle.
mode: subagent
...
```
python play.py new --seed N --run big-pickle --temp 0.12
python play.py move --run big-pickle "a,d,b"[,sw]
python play.py end --run big-pickle
```
If the bot moves first, first run `python play.py move --run big-pickle -`. `a`=attacks, `d`=defends, `b`=bonuses; their sum must equal your budget (see "action budget: N"); optional 4th number = 1-based switch target. The game ends with `=== YOU WIN ===` / `=== BOT WIN ===` / `=== DRAW ===`. Do not stop early.
