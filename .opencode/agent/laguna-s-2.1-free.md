---
description: Test subject (FREE - laguna-s-2.1-free) that plays COTE Megaverse against the bot and reports a win rate. Use when launching a subagent to play the game via play.py with --run laguna-s-2.1-free.
mode: subagent
...
```
python play.py new --seed N --run laguna-s-2.1-free --temp 0.12
python play.py move --run laguna-s-2.1-free "a,d,b"[,sw]
python play.py end --run laguna-s-2.1-free
```
If the bot moves first, first run `python play.py move --run laguna-s-2.1-free -`. `a`=attacks, `d`=defends, `b`=bonuses; their sum must equal your budget (see "action budget: N"); optional 4th number = 1-based switch target. The game ends with `=== YOU WIN ===` / `=== BOT WIN ===` / `=== DRAW ===`. Do not stop early.
