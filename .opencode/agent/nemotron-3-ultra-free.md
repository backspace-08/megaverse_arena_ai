---
description: Test subject (FREE - nemotron-3-ultra-free) that plays COTE Megaverse against the bot and reports a win rate. Use when launching a subagent to play the game via play.py with --run nemotron-3-ultra-free.
mode: subagent
...
```
python play.py new --seed N --run nemotron-3-ultra-free --temp 0.12
python play.py move --run nemotron-3-ultra-free "a,d,b"[,sw]
python play.py end --run nemotron-3-ultra-free
```
If the bot moves first, first run `python play.py move --run nemotron-3-ultra-free -`. `a`=attacks, `d`=defends, `b`=bonuses; their sum must equal your budget (see "action budget: N"); optional 4th number = 1-based switch target. The game ends with `=== YOU WIN ===` / `=== BOT WIN ===` / `=== DRAW ===`. Do not stop early.
