...
- `src/cote_megaverse/coevolution.py`: neural and Smart agents, opponent model,
  expectimax, co-evolution and fitness.
...
- `artifacts/`: ignored generated genomes, evolution results, logs and battles.
...
Current known verification state from the previous work:

- mechanics: `79 passed, 0 failed`;
- Smart tests: `21 tests passed`;
- champion/UI tests: `19 tests passed`;
...
2. Decide whether to voluntarily switch based on HP, type matchup, burst risk
   and genome parameters.
3. Compute attack/defend/bonus preference weights.
4. Enumerate every legal attack/defend/bonus allocation for the remaining
   budget.
5. Evaluate allocations with one-step expectimax.
6. Apply shield-deadlock breakthrough logic.
7. Cap bonus actions and move overflow to attack.
8. Occasionally swap some attack/defend allocation for unpredictability.
9. Shuffle and return `BattleAction` objects.

The expectimax explicitly models two phases:
...
reference opponents. The current production profile is:
...
Per Smart agent, the intended workload is:

- 30 current population games;
- approximately 25% Hall-of-Fame games;
- 9 reference groups x 8 games = 72 reference games;
- approximately 102 games per agent before any implementation-specific HoF
  rounding.
...
- at every Smart snapshot, `gen_stats[*]["fixed_validation"]` stores current
  matchup rates and the current champion's winrate against previous snapshot
  champions;
...
- `parameterized_ai/best_genome.npy`;
- `parameterized_ai/best_genome_<timestamp>.npy`;
- matching `best_genome_<timestamp>.json` metadata;
- `parameterized_ai/versions.json`;
- `coevolution_result.json` and `coevolution_stats.json`;
- training logs and battle directories.
...
the validation records and action/archetype summaries.
...
Do not delete user-generated genome, log or battle artifacts while fixing code.

Generated model, weight, log, metadata and battle artifacts are ignored by
Git. Source code, tests, handoff documentation and configuration remain
trackable. Do not add generated artifacts to commits unless explicitly asked.
...
5. Use `full_turn_logs` for complete statistics and battle records; use the
   rolling `turn_logs` only as bounded AI memory.
...
## Recommended Handoff Format After A Task

When finishing a substantial task, produce a short update for the next AI:
